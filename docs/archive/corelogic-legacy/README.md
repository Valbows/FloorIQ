...
- ✅ ATTOM API integration (API key auth)
...
- **APIs**: ATTOM Property API, Google Gemini API
...
# ATTOM API (Required for Phase 2 - Market Insights)
...
**Note**: Without ATTOM credentials, Agent #2 will use fallback logic (square footage-based estimates).
...
### 7. (Optional) Add ATTOM API Credentials

For real market data from Agent #2 (Market Insights Analyst):

1. **Get API Credentials**:
   - Sign up at [ATTOM Developer Portal](https://developer.attomdata.com/)
   - Request access to Property API
   - Obtain an API key (Free Trial supported)

2. **Update `.env` file**:
   ```bash
   ATTOM_API_KEY=your_real_attom_key
   ```
...
**Without ATTOM**: Agent #2 uses fallback logic (~$200/sqft estimates)
**With ATTOM**: Agent #2 provides live comps, AVM, and market trends
...
# Run ATTOM client tests
docker-compose exec backend pytest backend/tests/unit/test_attom_client.py -v
...
```bash
# Validate ATTOM API connectivity
...
🤖 Agent #1: Floor Plan Analyst (Gemini Vision)
   ...
   └─ Status: processing → parsing_complete (~5-10s)
         ↓
🤖 Agent #2: Market Insights Analyst (Gemini + ATTOM)
   └─ Fetches ATTOM data:
...
**Fallback Logic**: If ATTOM API is unavailable, Agent #2 uses square footage-based estimates (~$200/sqft) with clearly marked low confidence.
